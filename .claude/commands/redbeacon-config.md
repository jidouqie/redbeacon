...
全量配置入口触发后，**第一件事是问用户想怎么配**，不要默认闷头走对话、不要替他选。先把场景和推荐讲清楚，再让他拍板：

> 「配置有两种方式，先说下区别你再选：
>
> **① 网页向导（在电脑前推荐用这个）** — 我打开一个本地网页，AI / 飞书 / 代理一次性填完。密钥直接在浏览器里填、**不经过聊天框**，每项填完当场点按钮测连通，还有「复制飞书权限」「打开注册页」这些按钮帮你。**如果你现在就坐在电脑前，建议用它，最省事、隐私也好。**
>
> **② 跟我对话配** — 我一步步问、你一条条发给我，边配边给你解释每一项是干嘛的。适合想边了解边配、或者不方便开网页的时候。
>
> 你想用哪种？」

- **选①网页向导** → 跳「网页向导分支」。
- **选②对话** → 走下面的 A / B / C 三段对话流。
- 用户没明确选 → **不要擅自开配**，再问一次让他二选一（可顺带重申"在电脑前更推荐网页向导"）。
