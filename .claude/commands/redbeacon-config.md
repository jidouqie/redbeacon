...
**第 2 步 · 在消息结尾用 `AskUserQuestion` 工具给出可点选的选项**，让用户**点一下**就行、**不用打字**：

- 调 `AskUserQuestion`，一个问题，两个选项：
  - 选项 1：label「网页配置（推荐）」，description「打开本地网页一次性填完，密钥不经过聊天」 ← **推荐项放第一个**
  - 选项 2：label「对话配置」，description「我一步步问、你一条条发，边配边解释」
- 用户在卡片里点选，不必手打"网页"或"对话"。

**根据点选结果分流：**
- 选「网页配置」 → 跳「网页向导分支」。
- 选「对话配置」 → 走下面的 A / B / C 三段对话流。
- 用户点了「Other」自定义 → 按他写的意思归到上面两类之一；含糊就再 `AskUserQuestion` 问一次，别擅自开配。
