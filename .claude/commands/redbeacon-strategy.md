...
**用户只说"想调下策略/改改设置"、没说改哪个时，给编号菜单让他挑**（别一上来三段全问）：

```
想调哪块？
1. 账号定位 —— 赛道/受众/语气/开场/字数/禁词…
2. 文案风格 —— 全局写作指引 + 各类内容怎么写
3. 配图 —— 卡片配色 / AI 封面 / 图片模型 / 参考图
回数字就行，也可以直接说（比如「语气更犀利点」）。
```
...
## B 段：文案预设（全局文案提示词 + 各类内容写法）

文案写作指导分**两层**：
- **全局文案提示词（`copy_guide`，account 级）**——这个号**所有内容**通用的写作指引，改一次全类型生效。面板上叫「全局文案提示词」。
- **各类内容写法（`content_type.prompt_template`，按类型）**——某一类内容（干货科普/痛点解析…）专属的写法补充。

两层在生成时会**合并**注入每篇。用户说「改文案风格 / 文案不对劲（笼统）」多半是改**全局**；说「干货科普这类怎么写」才是改**某类**。
...
> 这正是面板里「全局文案提示词」那个框。**定位时就该写上**（见 `/redbeacon-locate`）；若用户反馈"还是空的 / 想改全局风格"，就在这里写/改。清空传 `{"copy_guide": ""}`。
