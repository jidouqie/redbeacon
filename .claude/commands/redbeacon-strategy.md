...
>
> **改完顺手提一句**：「想看效果就让我生成一篇试试。」想直接看产出就 `/redbeacon-generate` 生成一篇对比；想在操作台核一眼可 `redbeacon ui app --page 定位 --account-id {ID}`。
...
- 但**长模板反复比对微调，网页方案页最顺手**——`/redbeacon-plans` 会在合适时一句话把用户深链送进 `redbeacon ui app --page 方案 --account-id {ID}`：左边改模板、右边看这次真会发给 AI 的成品，改完保存回来接着生成。
