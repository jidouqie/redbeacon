...
## 想法共创：用户要参与「这一篇」怎么写（「我想写一篇」默认走这里）

用户说「我想写一篇 / 帮我写篇文案」这类**单篇**诉求时，**别闷头从选题库抽一条直接生成**——把用户拉进来一起定这一篇：
...
**把未用选题排成编号清单**（按内容类型分组，每条带一句话钩子），让用户回个数字就行：
...
> 这条我打算这么写：{你的落地想法}。你想强调什么、或者想换个角度吗？

- 用户认可 → 用它当「本篇想法」。
- 用户有自己的明确想法 → 以用户为准，帮他补全成一句清晰的落地要求。
- 用户没意见 → 也可以不带想法直接写。

**③ 注入想法生成（并把想法存回选题，重写也带着）**

```bash
# 立刻产这一篇：--idea 作为最高优先级注入文案提示词
redbeacon generate --account-id {ID} --topic "<这条选题原文>" --idea "<聊定的本篇想法>"
