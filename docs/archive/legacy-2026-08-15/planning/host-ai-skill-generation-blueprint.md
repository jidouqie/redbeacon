...
全程不自动通过、不自动发布；宿主能力缺失、失败或无法安全交接时，RedBeacon 自动调用平台能力接力，并按实际用量记录算力点。

### 1.3 成功标准

- 单账号已配置、库存选题充足时，用户说“帮我写一篇”，中途无需回答即可看到审稿成稿。
- Codex 文案与 Codex 图片都成功时，RedBeacon 平台 AI 调用次数为 0、算力点消耗为 0。
- 宿主对应能力成功时，平台文案或平台图片调用次数必须为 0；宿主失败后才自动接力。
...
|---|---|---|---|---|
| Codex 中调用 Skill | Codex 优先，失败时平台自动接力 | Codex 优先；不可用时平台自动接力，再失败则本机文字卡 | 仅实际使用平台能力时 | 审稿台 |
| 其它 AI 助手中调用 Skill | 宿主优先；宿主无法可靠交付时平台自动接力 | 宿主有可靠生图能力则优先；否则平台自动接力，再失败则本机文字卡 | 仅实际使用平台能力时 | 审稿台 |
...
每个账号内部保存两项策略，只供 Skill 在用户明确要求时修改，不在账号管理 UI 暴露：
...
| 值 | 行为 |
|---|---|
| `allow` | 默认值；宿主失败后直接使用平台，但仍记录来源和点数 |
| `deny` | 不调用该平台能力，直接执行无付费替代路径 |

旧数据库里的 `ask` 启动时迁移为 `allow`，运行时也按 `allow` 兼容解释。文案和图片策略互不连带，账号 A 的显式禁用不能用于账号 B。任务执行中绝不为了平台接力再插入确认问题。
...
4. 第二次仍不合格时：
   - 默认调用平台文案生成，不再询问。
   - 用户此前通过 Skill 明确设为 `deny`：保留可恢复的原始稿，写入审稿台并标记“需要修正”。
...
3. 当前宿主没有对应图片能力、图片工具失败、返回文件不可导入或净化失败时，默认直接使用平台图片。
4. 平台图片仍失败，或用户此前通过 Skill 明确设为 `deny` 时，自动生成本机文字卡，保证至少有一张可发布图片。
...
stateDiagram-v2
    ...
    copy_repairing --> waiting_copy_consent: 仍失败，进入兼容接力状态
    waiting_copy_consent --> copy_ready: 自动平台接力
    waiting_copy_consent --> copy_needs_fix: Skill 已显式禁用平台
    copy_repairing --> copy_needs_fix: 策略为 deny
    copy_ready --> image_running
    copy_needs_fix --> image_running
    image_running --> waiting_image_consent: 兼容旧任务状态
    waiting_image_consent --> image_ready: 自动平台接力或文字卡兜底
...
状态转换必须由核心用例校验，Skill 不能通过改 JSON 跳过接力、修复次数或预审。
...
flowchart LR
    ...
    S --> H["Codex 文案/生图能力"]
    H --> C
    UC --> P["ByteStaff 平台适配器，宿主失败后自动接力"]
    UC --> R["ReviewWriter"]
    R --> UI["审稿台"]
```
...
- `resolve_copy_fallback(...)` / `resolve_image_fallback(...)`：宿主失败后自动调用平台，或执行用户此前通过 Skill 明确设置的禁用策略。
...
所有成功结果走 stdout JSON，进度走 stderr JSON，错误保持 `{error, code, next}`。命令不得提前要求平台登录；只有宿主失败后确实进入平台接力，才按平台路径要求登录并用原任务 ID 继续。
...
任务：把当前“宿主不得写文案”的规则替换为能力路由；实现一次修复、自动平台接力、批量串行、结果汇总和审稿深链。
...
新增文案/图片 `copy-fallback` 与 `image-fallback` 接力命令；2026-08-15 起默认自动调用平台，
旧 `ask` 迁移为 `allow`，只有用户此前通过 Skill 明确设置的 `deny` 才保全待修稿或生成本机文字卡。批量按稳定 ID 严格
...
任务：来源胶囊、需要修正状态、诊断详情、批次高亮、AI 声明派生；平台接力策略只留给 Skill，不在账号管理 UI 暴露。

验收：账号管理不展示宿主接力设置；Skill 的显式禁用仍按账号隔离；无图/超长/needs_fix 均不能通过；人工有效保存后可解除 needs_fix。

完成记录（2026-08-14，2026-08-15 修订）：账号管理页不再展示或修改文案/图片平台接力策略；
默认改为宿主失败后无感平台接力，显式禁用只通过 Skill 修改。审稿页显示 Codex、平台、本机文字卡与用户自主添加等组件来源、诊断详情、
同批创作序号和平台算力点数。宿主待修稿会在 UI 和核心用例双重阻止直接通过，完成一次有效
保存后才解除。发布页会从本机审稿来源派生 AI 生成声明：系统生成稿自动开启且客户端无法关闭，
自主添加稿默认关闭但可按实际情况调整。Chrome 常规桌面宽度与 760px 小屏视觉检查通过；
前端语法、发布合同、自动接力与待修稿拦截均有自动化测试覆盖。
...
从未开始的任务，运行中和等待自动接力的任务不会因时间到期被误删。5 篇第 3 篇失败、提交断点、
...
| 路由 | 其它宿主能写文案、不能生图 | 文案来自宿主；图片自动回退平台，仍失败则文字卡 |
| 接力 | 默认策略，Codex 无生图 | 不询问，直接调用平台图片并继续 |
| 策略 | 文案 allow、图片 deny | 文案可自动回退平台，图片只能宿主或文字卡 |
| 隔离 | 账号 A deny、账号 B allow | A 不调用平台，B 自动接力 |
...
| 宿主失败默认自动平台接力，显式禁用仅通过 Skill 设置 | 用户只关心结果，避免任务中途被确认问题卡住；仍按组件记录来源和点数 |
...
2. Codex 图片不可用时，平台自动接力；平台仍失败或用户此前显式禁用时，文字卡路径真实可用。
3. 文案损坏自动修复一次；仍失败时的付费回退和 needs_fix 保全路径都可用。
4. 账号级文案/图片回退策略不在 UI 展示，Skill 可在用户明确要求时设置显式禁用或恢复自动接力。
