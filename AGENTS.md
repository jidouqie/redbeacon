...
- **统一下载入口**：对外产品页是 `https://bytestaff.jiomig.com/market/redbeacon`。canonical manifest 固定为中央上海 OSS `https://bytestaff-download-releases.oss-cn-shanghai.aliyuncs.com/projects/redbeacon/<channel>/latest.json`；大文件先试统一下载节点一次，失败后回落 manifest 中不可变的中央 OSS URL。
