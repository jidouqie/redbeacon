...
- 中央 OSS 按项目、通道和可选 component **最多保留最近 3 个完整版本**，且必须始终包含 canonical `latest.json` 当前指向的版本；下载节点仍只保留 current。发布第 4 个版本后，发布任务必须通过全局 Skill 的受控保留流程列出将删版本和字节数，取得当次明确确认后才删除最旧版本，并在删除后重新验证 canonical、剩余版本数和当前节点/OSS 下载。项目代码不得自行获得或调用 OSS 删除权限。
