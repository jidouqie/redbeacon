...
- **官网安装、更新、卸载入口的 channel 同样是不可变身份**：正式入口字节只能固定 `stable`，测试入口字节只能固定 `test`。公开 wrapper 必须零参数、在联网前拒绝全部额外参数、逐字内置本通道中央 canonical URL 和 channel 参数；不得读取任何调用者环境、旧安装状态、缓存或测试模式来选择通道、canonical、数据/令牌/浏览器/skill/临时目录。测试换源只能修改不进入制品的临时 wrapper 副本，不能在公开脚本中留下 loopback 或 test-mode 后门。共享实现只能是非公开 internal core，由两个通道各自的固定 wrapper 显式传入 channel；正式制品树不得携带测试公开入口，测试制品树不得携带正式公开入口。构建必须生成 v3 `metadata/channel-identity.json` 和 v2 `metadata/channel-isolation-receipt.json`，同时绑定入口 SHA、冻结包 SHA/包内通道，以及 Windows/macOS 各自真实的冻结包 smoke 与安装事务 smoke 共四份原始报告 SHA；smoke 必须覆盖相反通道、`testing`/`beta` 别名、伪造更新源和路径、调用者环境保持、两通道数据/令牌/浏览器/skill/快捷方式/进程共存。任何标记、自报 `passed` 或未绑定真实制品的 JSON 都不算证据；项目检查和全局发布 Skill 任一层证据不完整即禁止上传和切换 canonical。
