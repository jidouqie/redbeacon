...
RedBeacon 是一个小红书运营数字员工：以本机客户端为主入口，把多账号定位、对标学习、选题、内容方案、创作、审稿、发布、归档和账号级自动化值守连成一套运营与获客系统；CLI 与 AI skill 共同操作同一份本地业务数据。以后维护本项目，优先按本文件判断；如果旧文档和本文件冲突，以本文件、全局 `bytestaff-digital-employee-publish` Skill、安装脚本和 CLI updater 代码为准。

## 当前事实

- **当前主路径是本机客户端**：UI 是主入口，CLI 和 skill 是给 AI 助手调用的能力入口。用户即便没有 AI 助手，也应该能直接打开客户端完成主要流程。
- **业务数据全在本机**：账号资料、选题、待审稿、归档都在 `~/.redbeacon`；测试版在 `~/.redbeacon_test`。客户端只有这一个数据源，没有源切换开关；早期的飞书云端源连同它的端口、适配器和配置项已在 0.1.123 整体删除，不是搁置，不要按“可恢复的可选后端”来设计。
- **平台只负责身份、AI 能力与付费动作结算**：设备令牌正式版在 `~/.bytestaff`，测试版在 `~/.bytestaff_test`。客户端不保存上游模型 key；小红书发布使用平台动态点数配置和“预留 → 成功结算 / 失败取消”协议，但账号运营数据仍只在本机。
- **账号级自动化已经是当前能力**：每个账号可以独立设置开始时间、每日批次、批次间隔、每批篇数、自动补题、自动通过和自动发布。调度只在客户端运行期间生效；任一自动化开启时客户端防止系统空闲休眠和熄屏，退出客户端或关闭自动化即释放，错过的时间点不补跑。
- **公开仓与专有内核分离**：公开根仓提供 skill、安装/卸载脚本、公开文档和不含密钥的构建/发布辅助内容；`cli/` 专有客户端内核由 `.gitignore` 排除并保存在私有仓库，只允许把封装后的 Windows/macOS 制品交给发布系统，不能把核心源码推入公开仓。
...
- **源码客户端入口固定**：本机开发客户端只能从仓库根目录运行 `tools/run_desktop_dev.sh stable`（正式数据）或 `tools/run_desktop_dev.sh test`（测试数据）。该入口固定 `cli/.venv`、隔离全局 Python 并清理跨通道环境变量；不得直接用系统 Python、全局 `redbeacon` 命令或已安装旧客户端冒充当前源码客户端。

## 主流程

当前人工链路是：

**安装 / 更新 -> 平台登录 -> 建账号 -> 小红书扫码登录 -> 定位 / 对标学习 -> 选题 / 内容方案 -> 生成或自主入审 -> 本机审核 -> 发布 / 归档**

当前自动化链路是：

**按账号开启值守 -> 到点检查账号与库存 -> 不足时补题 -> 串行生成 -> 预审并按配置通过 -> 按配置发布 -> 留存执行记录**

- 审核改稿在客户端操作台或对话命令里完成，读写的都是本机审核表。
- 生成和发布既可由用户主动触发，也可由账号级自动化在客户端运行期间触发；自动化不是操作系统后台服务，客户端退出后不会继续运行。
- 自动化复用人工入口的同一套 core usecase、串行生成队列、审稿硬校验、登录复验、发布计费和归档逻辑，不得另写一套宽松流程绕过检查。
- 小红书原生定时字段如果存在，是提交给小红书侧，不是 RedBeacon 自己常驻调度。
- AI 生成声明是用户可逐篇开关的发布选项，默认开启；显式关闭时前端、后端和实际提交必须尊重该选择，不能按生成来源强制改回开启。声明开关不改写内容内部真实生成来源与归档来源字段；界面提示用户按实际内容和平台要求选择。
- 小红书发布结果只有「发布成功 / 发布失败」：未检测到明确的成功页面或成功提示，含提交等待超时、点击异常，本次结果统一为「发布失败」并返回具体错误及可取得的诊断截图；不新增「待确认」中间态，不退回「未审核」。发布失败不撤销原审核通过，原稿自动留在待发布列表，携带上次失败原因；原选题、正文、配图不丢弃、不重新生成、不重复审核、不成功结算。提交后的错误先返回定位和修复，不在同次任务里自动重放提交。已发布成功但本地归档出错不属于发布失败，不得回待发布导致重复发布。单篇、批量、自动化、CLI 与桌面端共用此规则。
...
- **安装入口是受用户保护的稳定接口，默认禁止修改**：`install/install.ps1`、`install/install-test.ps1`、`install/install.sh`、`install/install-test.sh`，以及 CLI updater、README、官网中对外展示的安装 URL 和外层命令，除非用户在当前任务中明确授权修改，否则任何 agent 都只能读取、比较、诊断和测试，不得编辑、重写、拆分、合并、替换或“顺手优化”。用户要求修复正式版/测试版混装、版本判断、更新、打包、发布、下载、客户端或其他问题，均不自动构成修改安装入口的授权；问题能在别处修就只能在别处修。即使 agent 认为安装器需要安全加固、架构统一、代码复用或测试适配，也必须先停下，向用户说明必须改动的具体文件、具体行为和影响，取得明确同意后才能动手；测试失败也不能成为自动改写安装脚本的理由。
- **获准修改时仍必须保留既有安装策略，且只做用户点名问题的最小补丁**：Windows 长期命令、`-ExecutionPolicy Bypass` 调用方式和单个 PowerShell 进程安装策略不得改变；macOS 长期 `curl ... | bash` 命令、单个 shell 进程安装策略和现有 quarantine 清理不得改变；正式/测试官网 URL 不得改变。禁止重新引入运行时 `install-core.*` 下载、第二个 PowerShell / `pwsh` / Bash 安装阶段或其他会改变系统行为模型的链路。确需改变上述任何一项时，必须获得用户对该具体变化的再次明确授权，不能用对其他问题的授权代替。
...
- **官网安装、更新、卸载入口的 channel 同样是不可变身份**：正式入口字节只能固定 `stable`，测试入口字节只能固定 `test`。公开入口必须零参数、在联网前拒绝全部额外参数、逐字内置本通道中央 canonical URL 和 channel；不得读取任何调用者环境、旧安装状态、缓存或测试模式来选择通道、canonical、数据/令牌/浏览器/skill/临时目录。Windows 与 macOS 的公开**安装**入口必须自身包含完整安装实现并在当前 PowerShell / shell 进程内完成，禁止运行时再下载 `install-core.*`，也禁止再启动第二个 PowerShell、`pwsh` 或 `/bin/bash` 执行安装。正式/测试安装入口可以在构建前从同一受控源机械生成，但发布字节必须各自单体、固定通道；用户长期使用的官网 URL 与外层命令不得改变。卸载入口暂可继续委托非公开 `uninstall-core.*`。测试换源只能修改不进入制品的临时入口副本，不能在公开脚本中留下 loopback 或 test-mode 后门；正式制品树不得携带测试公开入口，测试制品树不得携带正式公开入口。构建必须生成 v3 `metadata/channel-identity.json` 和 v2 `metadata/channel-isolation-receipt.json`，同时绑定完整入口 SHA、冻结包 SHA/包内通道，以及 Windows/macOS 各自真实的冻结包 smoke 与安装事务 smoke 共四份原始报告 SHA；smoke 必须覆盖相反通道、`testing`/`beta` 别名、伪造更新源和路径、调用者环境保持、两通道数据/令牌/浏览器/skill/快捷方式/进程共存，并直接证明安装入口没有请求 `install-core.*`、没有生成第二个 shell 进程。任何标记、自报 `passed` 或未绑定真实制品的 JSON 都不算证据；项目检查和全局发布 Skill 任一层证据不完整即禁止上传和切换 canonical。
...
- `docs/archive/` 只保存历史资料，不属于当前项目上下文。日常开发、搜索、诊断、构建和发布不得读取或引用其中内容；只有用户明确要求追溯或恢复某项历史方案时才允许打开，并且恢复前必须重新对照当前代码与契约。
...
- 遇到旧文档提到飞书（无论说成主链路还是“已搁置的可选源”）、旧官网、GitHub 发布源、Cloud Code/Claude Code 作为唯一维护方式时，按历史资料处理，不要照搬到当前实现。
