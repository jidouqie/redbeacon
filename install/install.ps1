# ------------------------------------------------------------------------------
# RedBeacon stable installer (Windows). Run in PowerShell:
#     irm 'https://bytestaff.jiomig.com/redbeacon/install.ps1' | iex
#
# This public installer is intentionally monolithic: it fetches the fixed release
# manifest and installs the bundle in this PowerShell process.
...
$script:InstallerEnvironmentKeys = @(
  "HOME", "USERPROFILE", "LOCALAPPDATA", "APPDATA", "PATH", "TEMP", "TMP", "TMPDIR", "PSMODULEPATH",
  "REDBEACON_CHANNEL", "REDBEACON_BUILD_CHANNEL", "REDBEACON_UPDATE_URL",
  "REDBEACON_DATA_DIR", "REDBEACON_LOG_DIR", "BYTESTAFF_HOME",
  "REDBEACON_PLAYWRIGHT_DIR", "REDBEACON_CLOAKBROWSER_DIR",
  "PLAYWRIGHT_BROWSERS_PATH", "PLAYWRIGHT_NODEJS_PATH",
  "CLOAKBROWSER_CACHE_DIR", "CLOAKBROWSER_BINARY_PATH", "CLOAKBROWSER_VERSION",
  "CLOAKBROWSER_SKIP_CHECKSUM", "CLOAKBROWSER_AUTO_UPDATE"
)
$script:OriginalInstallerEnvironment = @{}
foreach($key in $script:InstallerEnvironmentKeys){
  $script:OriginalInstallerEnvironment[$key] = [Environment]::GetEnvironmentVariable($key, "Process")
}

function Restore-CallerWindowsInstallerEnvironment(){
  foreach($key in $script:InstallerEnvironmentKeys){
    [Environment]::SetEnvironmentVariable($key, $script:OriginalInstallerEnvironment[$key], "Process")
  }
}

function Initialize-TrustedWindowsInstallerEnvironment(){
  $script:TrustedUserProfile = [Environment]::GetFolderPath([Environment+SpecialFolder]::UserProfile)
  $trustedLocalAppData = [Environment]::GetFolderPath([Environment+SpecialFolder]::LocalApplicationData)
  $trustedAppData = [Environment]::GetFolderPath([Environment+SpecialFolder]::ApplicationData)
  $trustedSystemDirectory = [Environment]::SystemDirectory
  foreach($entry in @(
    @("User profile", $script:TrustedUserProfile),
    @("Local application data", $trustedLocalAppData),
    @("Application data", $trustedAppData),
    @("Windows system directory", $trustedSystemDirectory)
  )){
  ...
  $trustedPowerShellDirectory = [IO.Path]::Combine($trustedSystemDirectory, "WindowsPowerShell", "v1.0")
  $script:TrustedTarExe = [IO.Path]::Combine($trustedSystemDirectory, "tar.exe")
  if(-not [IO.File]::Exists($script:TrustedTarExe)){ throw "Trusted Windows tar.exe was not found." }
  $trustedTemp = [IO.Path]::Combine($trustedLocalAppData, "Temp")
  [void][IO.Directory]::CreateDirectory($trustedTemp)
  $trustedValues = @{
    "HOME" = $script:TrustedUserProfile
    "USERPROFILE" = $script:TrustedUserProfile
    "LOCALAPPDATA" = $trustedLocalAppData
    "APPDATA" = $trustedAppData
    "PATH" = "$trustedSystemDirectory;$trustedWindowsDirectory;$trustedPowerShellDirectory"
    "TEMP" = $trustedTemp
    "TMP" = $trustedTemp
    "TMPDIR" = $trustedTemp
    "PSMODULEPATH" = [IO.Path]::Combine($trustedPowerShellDirectory, "Modules")
  }
  foreach($entry in $trustedValues.GetEnumerator()){
  ...
  $env:REDBEACON_BUILD_CHANNEL = "stable"
}

try {
Initialize-TrustedWindowsInstallerEnvironment
...
function Start-InstalledApp([string]$GuiPath, [string]$WorkingDirectory){
  if($RedBeaconInstallerExecutionMode -eq "smoke"){ return }
...
function Write-CliLauncher(){
  New-Item -ItemType Directory -Force -Path $BinDir | Out-Null
  # Keep the batch file ASCII; cmd.exe expands the Unicode user path at run time.
  Set-Content -LiteralPath (Join-Path $BinDir "$CmdName.cmd") -Encoding ASCII -Value @("@echo off", "`"%LOCALAPPDATA%\Programs\$AppName\$CliName.exe`" %*")
}
...
function Assert-ReleaseUrl([string]$Url, [string]$Label){
  ...
  $safeSmokeOrigin = (
    $RedBeaconInstallerExecutionMode -eq "smoke" -and
    -not $uri.UserInfo -and -not $uri.Query -and -not $uri.Fragment -and
    $Url.StartsWith($script:ReleaseOrigin + "/", [System.StringComparison]::Ordinal)
  )
  if(-not $safeHttps -and -not $safeSmokeOrigin){ Die "$Label URL is unsafe." }
  return $uri
}
function Get-ReleaseArtifact($Manifest, [string]$Path){
  ...
  $expectedUrl = "$($script:ReleaseOrigin)/projects/redbeacon/$Channel/releases/$($script:ReleaseVersion)/$Path"
  if($ossUrl -cne $expectedUrl){ Die "Artifact URL does not match RedBeacon $Channel release: $Path" }
...
function Test-ArtifactFile([string]$Path, $Artifact){
  ...
  return ((Microsoft.PowerShell.Utility\Get-FileHash -Algorithm SHA256 -LiteralPath $Path).Hash.ToLowerInvariant() -eq ([string]$Artifact.sha256).ToLowerInvariant())
}
...
function Download-ReleaseArtifact($Artifact, [string]$OutFile){
  ...
  foreach($attempt in 1..3){
    try {
      Microsoft.PowerShell.Utility\Invoke-WebRequest -Uri $ossUrl -OutFile $OutFile -UseBasicParsing -TimeoutSec 600
...
function Get-AssistantSkillRoots(){
  return @(
    [pscustomobject]@{ Name = "codex"; Path = Join-Path $script:TrustedUserProfile ".codex\skills" },
    [pscustomobject]@{ Name = "openclaw"; Path = Join-Path $script:TrustedUserProfile ".openclaw\skills" },
    [pscustomobject]@{ Name = "hermes"; Path = Join-Path $script:TrustedUserProfile ".hermes\skills" },
    [pscustomobject]@{ Name = "workbuddy"; Path = Join-Path $script:TrustedUserProfile ".workbuddy\skills" }
  )
}
...
function Run-BrowserSetup($CliPath){
  ...
  try {
    & $CliPath setup --manifest-file $script:ReleaseManifestFile
...
function Prepare-Skills($TempDir){
  ...
  foreach($t in 1..3){
    try {
      ...
      Remove-Item -Recurse -Force $skillStage -ErrorAction SilentlyContinue
      New-Item -ItemType Directory -Force -Path $skillStage | Out-Null
      & $script:TrustedTarExe -xzf $star -C $skillStage
  ...
  if($SkillVersion){
    ...
    try { $skillMeta = Get-Content -Raw -Encoding UTF8 -Path $metaPath.FullName | Microsoft.PowerShell.Utility\ConvertFrom-Json }
...
function Install-Skills($CliPath, $TempDir){
  ...
  $sourceSkills | ForEach-Object {
    ...
    $sourceHash = (Microsoft.PowerShell.Utility\Get-FileHash -Algorithm SHA256 -Path $_.FullName).Hash
    $destHash = (Microsoft.PowerShell.Utility\Get-FileHash -Algorithm SHA256 -Path (Join-Path $SkillDest $_.Name)).Hash
    ...
    $portableHash = (Microsoft.PowerShell.Utility\Get-FileHash -Algorithm SHA256 -Path $portableSource).Hash
    foreach($assistant in Get-AssistantSkillRoots){
      ...
      if((Microsoft.PowerShell.Utility\Get-FileHash -Algorithm SHA256 -Path $target).Hash -ne $portableHash){
...
$script:CanonicalManifestUrl = "https://bytestaff-download-releases.oss-cn-shanghai.aliyuncs.com/projects/redbeacon/stable/latest.json"
if($RedBeaconInstallerExecutionMode -eq "smoke"){
  [Console]::Out.WriteLine("BYTESTAFF_SMOKE_CORE_CHANNEL=$Channel")
}
if($script:CanonicalManifestUrl -cne "$($script:ReleaseOrigin)/projects/redbeacon/$Channel/latest.json"){
  Die "The fixed RedBeacon $Channel manifest URL is invalid."
}
[void](Assert-ReleaseUrl $script:CanonicalManifestUrl "Manifest")
if($Channel -eq "test"){
  $AppName = "RedBeacon_test"
  $CmdName = "redbeacon-test"
  $CliName = "redbeacon-test-cli"
  $DefaultSkillDest = "$script:TrustedUserProfile\.claude\commands-redbeacon-test"
  $RuntimeRoot = "$script:TrustedUserProfile\.redbeacon_test"
} else {
  $AppName = "RedBeacon"
  $CmdName = "redbeacon"
  $CliName = "redbeacon-cli"
  $DefaultSkillDest = "$script:TrustedUserProfile\.claude\commands"
  $RuntimeRoot = "$script:TrustedUserProfile\.redbeacon"
}
...
$BinDir = "$script:TrustedUserProfile\.local\bin"
$cliExe = Join-Path $Dest "$CliName.exe"
$tmp = New-Item -ItemType Directory -Force -Path (Join-Path $env:TEMP ("rb_" + [guid]::NewGuid()))

try {
  # 1) Fetch the fixed-channel manifest in this process, then skip the large
  # bundle when the installed client is already current.
  $script:ReleaseManifestFile = Join-Path $tmp "latest.json"
  try {
    Microsoft.PowerShell.Utility\Invoke-WebRequest -Uri $script:CanonicalManifestUrl -OutFile $script:ReleaseManifestFile -UseBasicParsing -TimeoutSec 20 | Microsoft.PowerShell.Core\Out-Null
    $manifestText = [System.Text.UTF8Encoding]::new($false, $true).GetString(
      [System.IO.File]::ReadAllBytes($script:ReleaseManifestFile)
    )
    $manifest = $manifestText | Microsoft.PowerShell.Utility\ConvertFrom-Json
  }
  catch { Die "The fixed RedBeacon stable manifest is not valid UTF-8 JSON." }
  if(([string]$manifest.schema) -ne "1" -or ([string]$manifest.project) -ne "redbeacon" -or ([string]$manifest.channel) -ne $Channel){
  ...
  if(-not $env:REDBEACON_FORCE_INSTALL -and $latest -and $current -eq $latest){
    ...
    if($healthy){
      ...
      # Refresh launchers too, so a healthy repeat install repairs older shims.
      Write-CliLauncher
      Commit-Skills
  ...
  Say "[2/4] Installing ..."
  $ex = Join-Path $tmp "x"
  Microsoft.PowerShell.Archive\Expand-Archive -Path $zip -DestinationPath $ex -Force
...
finally { Restore-CallerWindowsInstallerEnvironment }
