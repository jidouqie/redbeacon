#!/bin/sh
# Fixed test-channel bootstrap. It never delegates to the public stable
# installer name; both channels download the shared install-core.sh artifact.
...
[ "$core_url" = "$expected_core_url" ] \
  || die "Installer core URL does not match the test release."

fetch_file "$core_url" "$CORE_FILE" 60 || die "Could not fetch the test installer core."
...
/bin/bash "$CORE_FILE" "test" "$MANIFEST_FILE" "$CENTRAL_ORIGIN" "production"
