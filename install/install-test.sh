#!/usr/bin/env bash
# ------------------------------------------------------------------------------
# RedBeacon test installer (macOS Apple Silicon). Run:
...
# manifest and installs the bundle in this Bash process.
...
TRUSTED_HOME="$(resolve_trusted_home)" \
  || { printf 'xx Could not resolve the signed-in macOS user home directory.\n' >&2; exit 1; }
case "$TRUSTED_HOME" in /*) ;; *) printf 'xx The trusted macOS home directory is invalid.\n' >&2; exit 1 ;; esac
[ -d "$TRUSTED_HOME" ] \
  || { printf 'xx The trusted macOS home directory does not exist.\n' >&2; exit 1; }
HOME="$TRUSTED_HOME"
...
unset BASH_ENV ENV CDPATH GLOBIGNORE TAR_OPTIONS UNZIP UNZIPOPT ZIPOPT \
  ...
  PYTHONHOME PYTHONPATH CURL_CA_BUNDLE SSL_CERT_FILE \
  ...
  CLOAKBROWSER_SKIP_CHECKSUM CLOAKBROWSER_AUTO_UPDATE 2>/dev/null || true
umask 077

CHANNEL="test"
RELEASE_ORIGIN="https://bytestaff-download-releases.oss-cn-shanghai.aliyuncs.com"
MANIFEST_URL="https://bytestaff-download-releases.oss-cn-shanghai.aliyuncs.com/projects/redbeacon/test/latest.json"
EXECUTION_MODE="production"

# The fixed public channel is the sole identity source. Replace hostile or
# stale caller values before any bundled executable is started.
...
if [ "$CHANNEL" = "test" ]; then
  ...
  SKILL_DEST="$HOME/.claude/commands-redbeacon-test"
  ...
  SKILL_DEST="$HOME/.claude/commands"
...
OPENCLAW_SKILL_DIR="$HOME/.openclaw/skills"
HERMES_SKILL_DIR="$HOME/.hermes/skills"
WORKBUDDY_SKILL_DIR="$HOME/.workbuddy/skills"

say()  { printf '\033[36m==> %s\033[0m\n' "$*"; }
warn() { printf '\033[33m!! %s\033[0m\n' "$*"; }
die()  { printf '\033[31mxx %s\033[0m\n' "$*" >&2; exit 1; }

fetch_fixed_manifest() {
  /usr/bin/curl -q -fsSL --proto '=https' --proto-redir '=https' \
    --connect-timeout 3 --max-time 20 "$MANIFEST_URL" -o "$TMP/latest.json"
}

launch_installed_app() {
  app="$1"
  [ "$EXECUTION_MODE" = "production" ] || return 0
...
is_allowed_download_url() {
  case "$1" in
    https://*) return 0 ;;
    "$RELEASE_ORIGIN"/*) [ "$EXECUTION_MODE" = "smoke" ] ;;
    *) return 1 ;;
  esac
}
...
download_release_artifact() {
  ...
  expected_oss_url="$RELEASE_ORIGIN/projects/redbeacon/$CHANNEL/releases/$LATEST/$artifact_path"
  [ "$oss_url" = "$expected_oss_url" ] \
    || die "Artifact URL does not match RedBeacon $CHANNEL release: $artifact_path"
...
EOF
  ...
  if [ -n "$node_url" ]; then
    is_allowed_download_url "$node_url" || die "Invalid download node URL for $artifact_path."
    node_ok=""
    ...
    if [ "$EXECUTION_MODE" = "smoke" ] && [ "${node_url#"$RELEASE_ORIGIN"/}" != "$node_url" ]; then
...
install_skills() {
  ...
  for f in "$PREPARED_SKILL_SRC"/redbeacon*.md; do
    ...
    stem="$(basename "$f" .md)"
    skill_matches_channel "$stem" || continue
    cp -f "$f" "$SKILL_DEST"/ || die "Could not install the matching skill: $stem"
    copied_claude=1
  done
  [ -n "$copied_claude" ] || die "Prepared skill bundle has no Claude skill for channel $CHANNEL."
...
run_browser_setup() {
  ...
  if REDBEACON_CHANNEL="$CHANNEL" \
     ...
     REDBEACON_OUT=compact "$cli" setup --manifest-file "$TMP/latest.json"; then
...
TMP="$(/usr/bin/mktemp -d /private/tmp/redbeacon-test-install.XXXXXX)" \
...
HAD_EXISTING_APP=""
APP_BACKED_UP=""
APP_PLACEMENT_STARTED=""
cleanup_install() {
  if [ -z "$COMMITTED" ] && [ -n "$FINAL_PATH" ]; then
    # Until backup succeeds, FINAL_PATH still belongs to the old client.
    # Once placement starts it can contain a partial copy of the new client.
    if [ -n "$APP_PLACEMENT_STARTED" ]; then
      rm -rf "$FINAL_PATH" 2>/dev/null || true
    fi
    if [ -n "$APP_BACKED_UP" ] && [ -e "$BACKUP_PATH" ]; then
      if [ ! -e "$FINAL_PATH" ] && [ ! -L "$FINAL_PATH" ]; then
        mv "$BACKUP_PATH" "$FINAL_PATH" 2>/dev/null \
          || warn "Could not restore the old client. Its backup remains at $BACKUP_PATH."
      else
        warn "Could not remove the new client. The old client remains at $BACKUP_PATH."
      fi
    fi
    if [ -n "$APP_PLACEMENT_STARTED" ] && [ -z "$HAD_EXISTING_APP" ]; then
      rm -f "$BINDIR/$CMD_NAME" 2>/dev/null || true
      if [ "$OS" = "Linux" ]; then
        rm -f "$HOME/.local/share/applications/$DESKTOP_ID.desktop" 2>/dev/null || true
      fi
    fi
    if [ -n "$APP_BACKED_UP" ] && [ "$OS" = "Darwin" ] && [ -e "$FINAL_PATH" ]; then
...
# 2) Fetch the fixed-channel manifest in this process. Every selected artifact
# remains bound to this one release snapshot.
fetch_fixed_manifest \
  || die "Could not fetch the fixed RedBeacon test release manifest."
...
MANIFEST_SCHEMA="$(manifest_value "$TMP/latest.json" __manifest__ schema)"
LATEST="$(manifest_value "$TMP/latest.json" __manifest__ version)"
[ "$MANIFEST_PROJECT" = "redbeacon" ] && [ "$MANIFEST_CHANNEL" = "$CHANNEL" ] && [ "$MANIFEST_SCHEMA" = "1" ] \
  || die "The release manifest does not match RedBeacon $CHANNEL."
printf '%s\n' "$LATEST" | /usr/bin/grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+$' \
  || die "The release manifest has no valid version."
...
if [ "$OS" = "Darwin" ]; then
  ...
  rm -rf "$BACKUP_PATH" || die "Could not clear the previous client backup."
  if [ -e "$APP" ]; then
    HAD_EXISTING_APP=1
    mv "$APP" "$BACKUP_PATH" || die "Could not back up the existing client."
    APP_BACKED_UP=1
  fi
  APP_PLACEMENT_STARTED=1
