...
installer_url="$(/usr/bin/osascript -l JavaScript - "$tmp/latest.json" <<'JXA'
function run(argv) {
  const app = Application.currentApplication(); app.includeStandardAdditions = true;
  const data = JSON.parse(app.read(Path(argv[0])));
  const rows = (data.artifacts || []).filter(x => x.path === "installers/install.sh");
  if (rows.length !== 1) throw new Error("test installer is missing");
...
case "$installer_url" in
  http://127.0.0.1:*/*)
    [ "${REDBEACON_INSTALLER_TEST_MODE:-}" = "1" ] \
      || { echo "Unsafe test installer URL" >&2; exit 1; }
    curl -fsSL --connect-timeout 3 --max-time 60 "$installer_url" | bash
    ;;
  https://*)
