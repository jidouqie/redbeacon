# ------------------------------------------------------------------------------
# RedBeacon test installer (Windows). Run in PowerShell:
...
function Initialize-TrustedWindowsInstallerEnvironment(){
  ...
  $env:REDBEACON_BUILD_CHANNEL = "test"
}
...
$script:CanonicalManifestUrl = "https://bytestaff-download-releases.oss-cn-shanghai.aliyuncs.com/projects/redbeacon/test/latest.json"
...
try {
  ...
  catch { Die "The fixed RedBeacon test manifest is not valid UTF-8 JSON." }
