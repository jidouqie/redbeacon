#!/usr/bin/env bash
# ------------------------------------------------------------------------------
# RedBeacon installer core (macOS Apple Silicon).
# Invoked only by the fixed-channel install.sh / install-test.sh entrypoints.
...
if [ "$#" -ne 4 ]; then
  printf 'xx Internal installer requires channel, manifest, release origin and execution mode.\n' >&2
  exit 2
fi
CHANNEL="$1"
...
case "$CHANNEL" in
  stable|test) ;;
  *) printf 'xx Internal installer channel must be stable or test.\n' >&2; exit 2 ;;
esac
[ -f "$MANIFEST_SOURCE_FILE" ] \
  || { printf 'xx Internal installer manifest file is missing.\n' >&2; exit 2; }
case "$EXECUTION_MODE" in production|smoke) ;; *) printf 'xx Invalid internal installer execution mode.\n' >&2; exit 2 ;; esac
case "$RELEASE_ORIGIN" in
  https://*) ;;
  http://127.0.0.1:*) [ "$EXECUTION_MODE" = "smoke" ] || { printf 'xx Production release origin must use HTTPS.\n' >&2; exit 2; } ;;
  *) printf 'xx Internal installer release origin is invalid.\n' >&2; exit 2 ;;
esac
...
# 2) Fast no-op for repeat installs: the fixed-channel wrapper already fetched
# and authenticated the tiny manifest. Copy that exact snapshot so the core,
# its own hash, and every selected artifact remain bound to one release view.
cp "$MANIFEST_SOURCE_FILE" "$TMP/latest.json" \
  || die "Could not read the fixed-channel RedBeacon release manifest."
