# ------------------------------------------------------------------------------
# RedBeacon internal installer implementation (Windows).
# Public channel-owned wrappers pass the release channel and manifest explicitly.
...
# ------------------------------------------------------------------------------
param(
  [string]$RedBeaconInstallerChannel = "",
  [string]$RedBeaconInstallerManifestFile = "",
  [string]$RedBeaconInstallerReleaseOrigin = "",
  [ValidateSet("production", "smoke")]
  [string]$RedBeaconInstallerExecutionMode = "production"
)
...
function Get-AssistantSkillRoots(){
  return @(
    ...
    [pscustomobject]@{ Name = "openclaw"; Path = Join-Path $HOME ".openclaw\skills" },
    [pscustomobject]@{ Name = "hermes"; Path = Join-Path $HOME ".hermes\skills" },
    [pscustomobject]@{ Name = "workbuddy"; Path = Join-Path $HOME ".workbuddy\skills" }
  )
}
...
$Channel = $RedBeaconInstallerChannel.ToLowerInvariant()
if(@("stable", "test") -notcontains $Channel){ Die "Internal installer channel must be stable or test." }
if($RedBeaconInstallerExecutionMode -eq "smoke"){
  [Console]::Out.WriteLine("BYTESTAFF_SMOKE_CORE_CHANNEL=$Channel")
}
if([string]::IsNullOrWhiteSpace($RedBeaconInstallerManifestFile) -or -not (Test-Path -LiteralPath $RedBeaconInstallerManifestFile -PathType Leaf)){
  Die "Internal installer manifest file is required."
}
if([string]::IsNullOrWhiteSpace($RedBeaconInstallerReleaseOrigin)){ Die "Internal installer release origin is required." }
$script:ReleaseOrigin = $RedBeaconInstallerReleaseOrigin.TrimEnd("/")
try { $releaseOriginUri = [Uri]$script:ReleaseOrigin } catch { Die "Internal installer release origin is invalid." }
if($releaseOriginUri.UserInfo -or $releaseOriginUri.Query -or $releaseOriginUri.Fragment){ Die "Internal installer release origin is invalid." }
if($RedBeaconInstallerExecutionMode -eq "production" -and $releaseOriginUri.Scheme -ne "https"){
  Die "Production installer release origin must use HTTPS."
}
if($Channel -eq "test"){
...
try {
  # 1) Consume the exact manifest bytes verified by the public wrapper, then
  # skip the large bundle when the installed client is already current.
  try {
    $manifestText = [System.Text.UTF8Encoding]::new($false, $true).GetString(
      [System.IO.File]::ReadAllBytes($RedBeaconInstallerManifestFile)
    )
    $manifest = $manifestText | ConvertFrom-Json
  }
  catch { Die "Internal installer manifest file is not valid UTF-8 JSON." }
  $script:ReleaseManifestFile = [System.IO.Path]::GetFullPath($RedBeaconInstallerManifestFile)
