...
业务数据全部保存在本机，没有需要配置的云端表格或第二个数据源。平台只承担账号身份、AI 能力调度和发布结算，不保存用户的本地运营数据库，也不向客户端下发上游模型密钥。
...
```text
.claude/commands/       Skill 正文唯一真源（含通道占位符）
.agents/skills/         由上一目录生成的通用 Agent Skills 副本
