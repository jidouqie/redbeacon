...
## 第一步：先拿到用户脑中的全局想法，再进入发现对话

### 1.1 先选择沟通方式，不要上来就盘问细项

全新账号开始定位时，**第一轮禁止直接问赛道、受众、差异化等细项**。用户通常已经有一部分答案藏在心里；你如果不知道他的整体意图就逐项发问，会制造信息差和大量低效追问。

先只问这一个问题，让用户选择沟通方式：

> 开始前，你愿意先简单说说想做一个什么账号吗？想到多少说多少，不用整理完整；我会先理解你的整体想法，再只补关键缺口。
>
> 1. 我先简述整体想法（**推荐**）—— 大概做什么、想给谁看、希望这个账号帮你达成什么
> 2. 你逐题带我梳理 —— 我暂时说不清，你一次问我一个问题并给建议选项
>
> 回数字就行，也可以直接开始讲。

- 用户选 **1** 或直接开始描述 → 先完整听完并提炼他已经给出的信息，向用户用一小段话复述你对账号类型和目的的理解；后续只问真正缺失且会改变定位的关键项，**不得把他已经说过的内容换个说法再问一次**。
- 用户选 **2** → 再进入逐题引导，每轮仍只问一个问题，并给 2-3 个贴合上下文的建议选项。
- `$ARGUMENTS` 或当前对话里已经包含足够的账号类型、目的和整体构想 → 不再重复询问沟通方式；直接简短复述你的理解，然后从关键缺口开始。
- “简述”不是让用户填写一张表。不要一口气要求他回答“做什么、给谁看、怎么变现、有什么优势”等多个问题；示例只用于降低表达门槛，用户说几句自然语言即可。

### 1.2 根据已有信息补关键缺口

拿到整体想法后，**用自然对话逐步引导，每次只问一个问题**，根据回答追问或转下一个话题。要摸清下面五件事（顺序灵活），但用户已经讲清的直接沿用；每轮最多补其中一个，等用户答完再进入下一项：
...
用人话讲，让用户选一个当默认（以后每篇都能临时改）。**用户没有明确偏好时，统一优先推荐 `both`，不要再因为是认知、观点或方法类账号就默认推荐纯文字卡片。**只有用户明确说不要 AI 图、优先省算力点、或只要最稳定的纯文字表达时，才建议 `cards`。推荐 AI 封面时要直说会消耗算力点，不隐藏费用差异。

- **AI 封面 + 文字卡片**（`both`，**推荐**）：封面用 AI 生成（可带你的风格/人物），正文仍是易读的渲染卡片。兼顾封面吸引力和正文阅读体验，会消耗生图算力点。
- **纯文字卡片**（`cards`）：正文和封面都渲染成文字卡片。稳定、不消耗生图算力点，适合明确不要 AI 图的用户。
...
`both` 是无明确偏好时的推荐值；用户选择后可替换为 `cards` / `ai`。
