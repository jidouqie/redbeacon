...
>
> 🔴 **为什么这件事非 skill 不可**：网页操作台能管选题库（增删改、AI 补一批），但**它没有联网检索能力**——「按这个账号定位，去互联网上找此刻正在发生、且和它匹配的真实事件/热点/痛点，再针对性推荐选题」**只有 AI 助手（自带联网 + 判断）能做**。这正是「有 AI 助手就把上限用到极高」的头号体现。别把它降级成"随便凑几个选题"——**要联网、要贴当下、要对着定位**。
>
...
>
> 什么时候来：定位定完铺第一批 / `/source-command-redbeacon-generate` 预警选题快用完 / 用户想重铺 / **用户说"帮我想想写什么""不知道发啥"**。**入库即可用**（不用人工二次确认转入），由 generate 消费，无本地审核。
...
> **每条都必须是一份完整创作简报，不能只丢标题，也不能靠“账号里大概有”而留空。** Skill 联网选题和平台 AI 补题都必须显式填写以下 10 项：**内容类型、应用域、问题类型、切入角度、要点提纲、价值点、互动钩子、聚焦人群、配图方向、备注**。账号定位可以帮助你拟出本条默认值，但存进选题库的每条记录自身必须完整；没有特殊备注就写“无额外备注”。这样用户在 UI 里看到的是完整选题卡，后续写稿也不会因入口不同而缺信息。
...
[
  {"content":"选题1","content_type":"避坑提醒","application_domain":"乙方关系","problem_type":"反例",
   "angle":"反常识开头：客户越催你越要先对齐验收标准","outline":"验收标准写进合同;每周同步进度截图;留一条改稿边界;超范围就走变更单","value_point":"少扯三次皮","hook":"评论区问你被白嫖过几次","audience":"刚开始独立接单的设计师","image_dir":"合同边界前后对比卡片","note":"无额外备注"},
  {"content":"选题2","content_type":"工具测评安利","application_domain":"行业定价权","problem_type":"决策",
   "angle":"先戳痛点：报价永远比同行低还是接不到单","outline":"按价值分层报价;给三档套餐锚定;先谈交付物再谈钱","value_point":"形成能复用的报价框架","hook":"评论区说说你最怕哪种砍价","audience":"自由职业者和小团队负责人","image_dir":"三档报价结构图","note":"避免承诺固定涨价幅度"}
]
```

> 返回 `{"inserted":N,"total":M,"record_ids":[...]}`（重复的选题文本自动跳过）。`--require-complete` 会在写库前整批检查上述 10 项；任何一条缺字段，整批都不写，先补齐再执行，避免出现一半完整一半空白。字段都是自由文本，按这条选题的真实创作意图填写，不用往预设词上凑。
...
| 场景 | 关系 |
|---|---|
| 刚定位完账号 | 从 `/source-command-redbeacon-locate` 过来开第一场规划会（locate 第四步已先铺了一批种子选题进本地选题库） |
| generate 预警选题低 | `/source-command-redbeacon-generate` 会把你引到这儿补，而不是生产途中硬塞 |
