...
> - **全程人话**：给用户的回复不出现 /source-command-redbeacon-* 或 redbeacon xxx 这类命令名/斜杠（那是你后台执行的），用「我来帮你写一篇」这种说法；除非用户主动要命令，否则别提、别列。
...
>
> 上一步是定位（`/source-command-redbeacon-locate`）→ 选题（`/source-command-redbeacon-topics`），下一步是审核（对话里标 / 操作台审稿页），然后 `/source-command-redbeacon-publish` 发布。
...
> 📌 选题快用完了，只剩 {unused} 条。想补就开一场选题规划会：**`/source-command-redbeacon-topics`**。先接着写也行，见底前补上即可。
...
|---|---|
| 生成内容 | **本 skill** |
| 改文案风格/配图风格"预设"（影响以后每次写） | `/source-command-redbeacon-strategy` 或 `/source-command-redbeacon-panel`；要改成套的提示词模板去网页「方案」页 |
| 「这篇文案/图不对劲」找原因调参 | `/source-command-redbeacon-diagnose` |
| 补选题 / 重定位 | `/source-command-redbeacon-locate` |
| 发布已通过内容 | `/source-command-redbeacon-publish` |
| 审稿改稿标通过 | `/source-command-redbeacon-review`（我直接接：`review list`→标通过），或深链审稿页逐条改 |
...
- 生成后内容**自动进本机审核表**，接着走 `/source-command-redbeacon-review` 审。
