---
name: source-command-redbeacon-config
...
> 📦 **纯本机**：业务数据都在本地。配置只剩两件事：**平台登录（必需，管生图/计费）** + **代理（可选，多账号防关联）**。
...
>
> 审核与改稿都在本机（`/source-command-redbeacon-review` 或操作台审稿页）。**文案/生图带令牌走平台扣点——客户端不需要任何 AI key / 中转站。**
...
> 🔗 平台登录也有**独立 skill** `/source-command-redbeacon-login`（专管平台登录/查会员/退出）。本段是 onboarding 一条龙里顺带做的平台登录；用户单独要「登录/重登/退出平台」时走 `/source-command-redbeacon-login` 即可，两边底层都是 `redbeacon login`。
...
> 💤 **客户端只有本机存储，没有云端表这个选项**：审核/选题/发布数据全在本地，没有可配的东西，也没有数据源开关。别引导用户去绑表。用户主动问「能不能接云端表格」时，如实说一句「客户端现在只用你本机的数据，不接云端表格，直接用就行」，然后带回正常流程。
...
- 进到 `stage2` → 配置就绪。**按主入口「自动推进原则」，直接交棒 `/source-command-redbeacon-accounts` 建号**（建完会自动接扫码登录 → 定位，一路到 ready），别问「要不要建账号」。
