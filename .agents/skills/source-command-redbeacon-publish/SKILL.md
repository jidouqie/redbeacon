...
> 📦 **数据都在本机**：从**本机审核表**读标「通过」的记录发布，发布成功移入本机归档。
...
发布要同时满足：**① 平台已登录且余额满足平台当前发布费率（0 点时无需余额）、② 账号小红书登录态在线、③ 审核表里有标「通过」的记录**。
...
- **代理（多账号防关联）**：`proxy_auto_rotate` 未开启时正常直连；一旦开启，代理 API 必须已配置。每次发布前取一条新 IP，并默认用**当前账号已有 Cookie 的小红书浏览器**挂上该 IP 打开首页验证；打不开就重新取，最多连续尝试 5 条。5 条都不可用时本次发布明确失败，**绝不静默回退直连**。代理设置在客户端「设置」页完成，本 skill 不追问、不接触代理密钥。
